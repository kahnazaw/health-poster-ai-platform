...
عند أول تشغيل للمنصة، يجب إنشاء حساب المدير الأول:

1. **بعد تشغيل الخادم، افتح المتصفح وانتقل إلى:**
   ```
...
2. **املأ نموذج الإعداد:**
   - **اسم المدير:** اسم المدير (مثال: "مدير النظام")
   - **البريد الإلكتروني:** بريد المدير (مثال: `admin@health.gov.iq`)
...
- **عملية واحدة فقط:** بعد إنشاء أول مستخدم، لن تكون صفحة `/setup` متاحة
- **حماية من Race Conditions:** يتم استخدام Transactions في قاعدة البيانات
- **تشفير آمن:** كلمات المرور مشفرة باستخدام bcrypt (12 rounds)
- **لا يمكن تجاوز:** لا توجد طريقة لإنشاء حساب مدير آخر بعد الإعداد الأولي

### بعد الإعداد

بعد إنشاء الحساب بنجاح:
1. سيتم إعادة توجيهك إلى `/login`
2. سجل الدخول باستخدام البريد الإلكتروني وكلمة المرور التي أنشأتها
3. سيتم إعادة توجيهك تلقائياً إلى `/dashboard/admin`

## خطوات الإعداد التقنية
